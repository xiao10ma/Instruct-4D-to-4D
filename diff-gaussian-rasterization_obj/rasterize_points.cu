	...
	const bool debug)
{
  ...
  return std::make_tuple(rendered, out_color, out_objects, out_flow, out_depth, out_T, radii, geomBuffer, binningBuffer, imgBuffer, covs3D_com);
}
	...
	const bool debug) 
{
  ...
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dobjects, dL_dopacity, dL_dmeans3D, dL_dcov3D,
