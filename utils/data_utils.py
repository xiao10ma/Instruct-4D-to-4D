...
class CameraDataset(Dataset):
    ...
    def __init__(self, cam_time_image, viewpoint_stack, white_background, num_frame=300):
        ...
        self.cam_time_image = cam_time_image
        ...
        # tbar = tqdm(range(len(viewpoint_stack)))
        ...
        #         img = Image.open(viewpoint_cam.image_path).convert('RGB')
        #         viewpoint_image = self.transform(img)
        #     else:
        ...
        # self.frame_list = range(num_frame)
        # self.cam_list = range(self.num_cam)

        # for i_frame, frame in enumerate(tqdm(self.frame_list, desc='frame')):
        #     for i in tqdm(cam)
        
        # for idx, view in enumerate(tqdm(viewpoint_stack, desc='Reading RGBS')):
            
        

        # self.time_cam_image = torch.stack(results, 0) # [num_frame * num_cam, C, H, W]
        # print(self.time_cam_image.shape)
    ...
    def __getitem__(self, index):
        ...
        # else:
        #     viewpoint_image = viewpoint_cam.image
        viewpoint_image = self.cam_time_image[index]
